...
(function ($) {
  ...
  Suggest.prototype = {
    ...
    __keyup: function (e) {
      ...
      var specialChars = [
        38, 40, 37, 39, 17, 18, 9, 16, 20, 91, 93, 36, 35, 45, 33, 34, 144, 112,
        113, 114, 115, 116, 117, 118, 119, 120, 121, 122, 123, 145, 19,
      ];
    ...
    __lookup: function (q, $resultItems) {
      ...
      this.$element.trigger($.extend({ type: "suggest.lookup" }, this), [
        q,
        $resultItems,
      ]);
          ...
          if (typeof options.position == "string") {
            switch (options.position) {
              case "bottom":
                position.top =
                  $el.outerHeight() -
                  parseFloat($dropdownMenu.css("margin-top"));
                position.left = 0;
                position.right = 0;
                break;
              case "top":
                position.top = -(
                  $dropdownMenu.outerHeight(true) +
                  parseFloat($dropdownMenu.css("margin-top"))
                );
                position.left = 0;
                position.right = 0;
    ...
    return this.each(function () {
      ...
      if (typeof arg1 === "string") {
        if (arg1.length == 1) {
          ...
          if (arg2) {
            ...
            if (typeof arg2 === "string") {
              ...
              suggestions[arg1] =
                $.isArray(arg2) || typeof arg2 === "function"
                  ? { data: arg2 }
                  : arg2;

              // if key is existing, update options
              if (data && arg1 in data) {
                data[arg1].options = $.extend(
                  {},
                  data[arg1].options,
                  suggestions[arg1]
                );
          ...
          console.error(
            "you're not initializing suggest properly. arg1 should have length == 1"
          );
        }
      } else {
        ...
        if (!data) $this.data("suggest", createSuggestions(this, arg1));
          ...
          $this.data(
            "suggest",
            $.extend(data, createSuggestions(that, suggestions))
          );
        }
      }
    });
